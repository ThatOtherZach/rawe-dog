...
- For line-level purges where the file must survive (e.g. `.replit`), `git filter-branch --tree-filter 'sed -i "/^BAD_LINE/d" file' -- origin/main..HEAD` works; the final HEAD checkout updates the worktree copy too, so no separate worktree edit is needed. Keep `refs/original` until the push succeeds.
