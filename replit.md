...
Both apply **only to operator-key runs** — `settings.apiKey === $XAI_API_KEY` with no custom endpoint. BYOK or a custom endpoint always bypasses both; no `XAI_API_KEY` env means every run counts as BYOK.
