...
- **Speculative selection on row expand**: when the user expands a posting row, PostingsPage immediately fires a background `POST /api/generate` (`mode: "pass1"`) for that posting. If the call resolves before the user clicks "Generate kit", the result is stored in `sessionStorage` under the key `rdPrefetchPass1`. GeneratePage reads and clears this entry at the start of `runPass1` — if the posting ID matches, it skips the API call and jumps straight to the review stage. One slot only: expanding a different row aborts any in-flight fetch and clears the previous cache entry. Errors are treated as silent cache misses; the normal Pass 1 call runs as fallback.
...
- **Two lanes**: kit generation is **free** (no credit check on `/api/generate`, though the free-tier gates below can limit operator-key runs); a $1 search credit is required to refresh postings via `/api/postings/refresh` (when `RAWEDOG_CREDITS_ENFORCED=true`). Paying gets you speed + targeting (pre-scored postings), not gated output quality.

### Free-tier gates on kit generation (loss-leader controls)
Both apply **only to operator-key runs** — requests that supplied no session AI credentials (`X-AI-Key`/`X-AI-Endpoint` headers) and fell back to env `XAI_API_KEY`. BYOK headers always bypass both; no `XAI_API_KEY` env means every run counts as BYOK.
- **Kill switch** (`FREE_TIER_ENABLED=false`): refuses every generate mode (incl. pass1) with a plain JSON 403 telling the user to add their own key/endpoint in Settings. Checked before the quota gate; takes no quota reservation.
- **Free daily kit quota** (`FREE_DAILY_KIT_ENFORCED=true`): 1 kit per window per caller, keyed by a privacy-preserving fingerprint — HMAC-SHA256 with a window-rotating pepper (`SESSION_SECRET` + window bucket) over normalized Master Profile name/phone/city + truncated client IP (/24 v4, /48 v6). Only `{hash, count, ts}` is stored (`data/free-quota.json`, pruned > 2× window); fingerprints are unlinkable across windows. Applies to drafting modes only (pass1 is free). Reservation model: atomic check-and-increment at run start, refunded in a route-level `finally` on any non-success exit (failed/aborted/validation-rejected runs stay free). Client IP must be the **last** `x-forwarded-for` entry (proxy-appended; first entries are forgeable).
- `/api/health` returns a caller-specific `freeKit` block (`enforced`, `disabled`, `remaining`, `windowHours`, `resetsInHours`; never the fingerprint) which drives the Generate page banner; the privacy disclosure lives in Settings → About.
...
| `FREE_TIER_ENABLED` | No | Free-tier kill switch. Default on; `false` refuses ALL kit generation on the operator's key (403, all modes) — users must add their own API key or endpoint |
| `FREE_DAILY_KIT_ENFORCED` | No | `true` arms the free daily kit quota (1 kit per window per fingerprint) on operator-key runs (off by default) |
| `FREE_KIT_WINDOW_HOURS` | No | Free-kit quota + fingerprint-rotation window (default 24; invalid values fall back with a logged warning) |
...
**Settings page**: Two panels — **AI & LLM Settings** (per-session AI API key + endpoint, drafting/selection/verification model overrides, Save, Test connection) and **TheirStack** (API key for live job search). The AI key/endpoint are **per user/session**: stored only in the visitor's browser (localStorage `rawe.ai.key` / `rawe.ai.endpoint`), sent as `X-AI-Key`/`X-AI-Endpoint` headers on LLM-touching requests, never persisted server-side (an `AsyncLocalStorage` request context in `lib/ai-context.ts` carries them through the pipeline; startup scrubs any legacy key from `settings.json`). Clear removes the key from the browser immediately; model prefs and toggles remain operator-owned server settings.
...
- The credit gate is invisible when off: `SearchCreditsPanel` renders nothing and `/api/postings/refresh` skips the credit check unless `RAWEDOG_CREDITS_ENFORCED=true`. `/api/generate` never checks credits, but operator-key runs are subject to the free-tier gates (`FREE_TIER_ENABLED` kill switch, `FREE_DAILY_KIT_ENFORCED` daily quota) — both invisible when off and always bypassed by BYOK. Losing `SESSION_SECRET` invalidates every issued credit token (HMAC), even though ledger rows survive.
...
- `.replit` is a **tracked file** — never put secrets or API keys in its `[env]` block. A committed key blocks all GitHub pushes (secret-scanning rejects the whole range) and requires a history rewrite to fix. Real keys go in Replit Secrets or the app's Settings page. `XAI_BASE_URL`/`THEIRSTACK_BASE_URL` are URL hooks for e2e mock servers — set them per-run in the test command, never committed.
